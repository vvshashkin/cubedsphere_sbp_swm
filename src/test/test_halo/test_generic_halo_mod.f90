...
private
public   :: test_scalar_halo

contains

subroutine test_scalar_halo(nh,halo_width,staggering,halo_procedure_name, check_corners)

    ! use mpi

    integer(kind=4),   intent(in)  :: nh, halo_width
    character(len=*),  intent(in)  :: staggering, halo_procedure_name
    logical,           intent(in)  :: check_corners

    integer(kind=4), parameter     :: nz=3, ex_halo_width=8
    ...
    call create_halo_procedure(halo,domain,halo_width,halo_procedure_name)
    ...
    if(domain%parcomm%myid == 0) &
        print '(5A,I4,A,I2)', 'Scalar halo procedure test, staggering: ', staggering,  &
                              ', halo-procedure: ', halo_procedure_name,  &
                              ", nh = ", nh,                              &
    ...
    call halo_err(inface_err, inface_err_max, cross_edge_err, cross_edge_err_max, &
                  ...
                  domain%partition%nh, halo_width, domain%mesh_p, check_corners,  &
                  f_test, f_true)
...
subroutine halo_err(gl_inface_err, gl_inface_err_max, gl_cross_edge_err, gl_cross_edge_err_max, &
                    ...
                    nh, halo_width, mesh, check_corners, f1, f2)
...
end module test_generic_halo_mod
